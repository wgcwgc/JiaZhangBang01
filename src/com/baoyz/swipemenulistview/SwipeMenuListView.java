...
public class SwipeMenuListView extends ListView
{
	...
	public boolean onInterceptTouchEvent(MotionEvent ev )
	{
		...
		switch(action)
		{
			...
			case MotionEvent.ACTION_MOVE:
				...
				{
					// 每次拦截的down都把触摸状态设置成了TOUCH_STATE_NONE
					// 只有返回true才会走onTouchEvent 所以写在这里就够了
	...
	@SuppressLint(
	{ "ClickableViewAccessibility", "Recycle" })
	@Override
	public boolean onTouchEvent(MotionEvent ev )
	{
		...
		{
			case MotionEvent.ACTION_DOWN:
				int oldPos = mTouchPosition;// this is
				// wrong/这里不能这么写，mTouchPosition与oldPos是永远相等的
				// -1是初始值，根据是否等于-1，判断是否第一次按下
				if(mOldTouchPosition == -1)
				 ...
				 * mTouchView.isOpen()) { mTouchState = TOUCH_STATE_X;
				 * mTouchView.onSwipe(ev); return true; }
				 */
				// 这里的mOldTouchPosition才是上一次的position
				if(mTouchPosition == mOldTouchPosition && mTouchView != null && mTouchView.isOpen())
				{
					mTouchState = TOUCH_STATE_X;
					mTouchView.onSwipe(ev);
					return true;
